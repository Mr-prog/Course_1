...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        ArrayList<String> ignore = new ArrayList<>(Arrays.asList("s", "d")); // исключения
        System.out.println("Введите строку, " + "исключения: " + ignore);
        String [] data = in.nextLine().split("");
        ArrayList<String> result = new ArrayList<>(Arrays.asList(data));
        ArrayList<String> ignoredData = new ArrayList<>();
        for (int i = 0; i < data.length; i++) {
            if(ignore.contains(data[i])){
                ignoredData.add(String.valueOf(i));
                ignoredData.add(data[i]);
            }
        }
        result.removeAll(ignore);
        Collections.reverse(result);
        for (int i = 0; i < ignoredData.size(); i+=2) {
            result.add(Integer.parseInt(ignoredData.get(i)), ignoredData.get(i + 1));
        }
